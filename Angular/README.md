...
2. Copy `src/app/services` and `src/app/utils` into your application. Update types and variables in [src/app/utils/chart-data.ts](src/app/utils/chart-data.ts) to match field names in your data set. Replace all instances of `GridDataItem` with your grid data type (this example imports `GridDataItem` from `src/app/data/grid-data.ts`).
